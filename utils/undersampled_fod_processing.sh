...
do
	...
	if [ $bool_7T == 'True' ]; then
		echo "7T undersampling"
		path=$data_path/$subject/T1w/Diffusion_7T
		T1w_im=T1w_acpc_dc_restore_1.05.nii.gz
	else
		echo "3T undersampling"
		path=$data_path/$subject/T1w/Diffusion
		T1w_im=T1w_acpc_dc_restore_1.25.nii.gz
	fi
	echo $subject
	echo $path
    mkdir $path/$folder_name
	
	if [ $fully_sampled_process == 'True' ]; then
		...
		5ttgen fsl $path/../$T1w_im $path/../5ttgen.nii.gz -nocrop			
	fi
...
while read -r subject;
do
	echo Preparing Validation subjects
	if [ $bool_7T == 'True' ]; then
		echo "7T undersampling"
		path=$data_path/$subject/T1w/Diffusion_7T
		T1w_im=T1w_acpc_dc_restore_1.05.nii.gz
	...
	if [ $fully_sampled_process == 'True' ]; then
		...
		5ttgen fsl $path/../$T1w_im $path/../5ttgen.nii.gz -nocrop
		mrconvert $path/../5ttgen.nii.gz -coord 3 2 $path/../white_matter_mask.mif
		...
		fixel2peaks -number 11 $path/fixel_directory $path/fixel_directory/fixel_directions.mif
